...
struct Context {
  ...
  Context(Context &&other) noexcept
      : instance(other.instance), adapter(other.adapter),
        device(other.device), queue(other.queue),
        pool(std::move(other.pool)), kernelPool(std::move(other.kernelPool)) {
    // Set moved-from resource handles to nullptr so they won't be released.
    other.instance = nullptr;
    other.adapter = nullptr;
    other.device = nullptr;
    other.queue = nullptr;

    // Also, if your sub-objects (pool, kernelPool) depend on 'this', you might need to update them here.
    pool.ctx = this;
    kernelPool.ctx = this;
  }
  ...
  Context &operator=(Context &&other) noexcept {
    if (this != &other) {
      // Release our existing resources if any.
      // (You might want to invoke a cleanup function here before overwriting,
      //  or ensure that this Context is not in use.)
      instance = other.instance;
      adapter = other.adapter;
      device = other.device;
      queue = other.queue;
      pool = std::move(other.pool);
      kernelPool = std::move(other.kernelPool);
...
createTensor(TensorPool &pool, WGPUDevice &device, const Shape &shape,
  ...
  size_t totalSize = sizeBytes(dtype) * numElements;
  ...
  bufferDesc.label = nullptr;  // Optional: you can supply a string label if desired

  WGPUBuffer buffer = wgpuDeviceCreateBuffer(device, &bufferDesc);
  if (buffer == nullptr) {
      LOG(kDefLog, kError, "Failed to create tensor buffer");
      // Handle the error as needed (throw, return a default tensor, etc.)
  }
