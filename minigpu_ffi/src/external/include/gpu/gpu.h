...
namespace gpu
{
                               ...
                               const WGPUDeviceDescriptor &devDescriptor = {})
  {
    ...
    LOG(kDefLog, kInfo, "Requesting adapter");
    {
      ...
      // Create a RequestAdapterOptions structure
      ...
      // Create a RequestAdapterCallbackInfo structure
      auto adapterCallback = [](WGPURequestAdapterStatus status, WGPUAdapter adapter, char const *message, void *pUserData)
      {
      ...
      // Request the adapter synchronously
      wgpuInstanceRequestAdapter(context.instance, &options, adapterCallback, &adapterData);

      // Wait for the adapter request to complete
      while (!adapterData.requestEnded)
      {
        // Process events or yield execution if needed
        // For example: std::this_thread::yield();
      }

      if (adapterData.adapter == nullptr)
      {
    ...
    LOG(kDefLog, kInfo, "Requesting device");
    {
      ...
      // Create a RequestDeviceCallback
      auto deviceCallback = [](WGPURequestDeviceStatus status, WGPUDevice device, char const *message, void *pUserData)
      {
        DeviceData &devData = *reinterpret_cast<DeviceData *>(pUserData);
        if (status != WGPURequestDeviceStatus_Success)
        {
          LOG(kDefLog, kError, "Could not get WebGPU device: %s", message);
        }
        else
        {
      ...
      // Request the device synchronously
      wgpuAdapterRequestDevice(context.adapter, &devDescriptor, deviceCallback, &devData);
      ...
      while (!devData.requestEnded)
      {
        ...
        std::cout << "Waiting for device request to complete..." << std::endl;
      }

      if (devData.device == nullptr)
      {
                             ...
                             std::promise<void> &promise)
  {
    ...
    // Create a callback function that captures the shared_ptr
    auto callback = [](WGPUQueueWorkDoneStatus status, void *userData)
    {
      auto promisePtr = static_cast<std::shared_ptr<std::promise<void>> *>(userData);
      check(status == WGPUQueueWorkDoneStatus_Success, "Queue work done",
            __FILE__, __LINE__);
    ...
    // Pass the shared_ptr as user data to the callback
    auto userdata = new std::shared_ptr<std::promise<void>>(promisePtr);
    wgpuQueueOnSubmittedWorkDone(ctx.queue, callback, userdata);
  }
} // namespace gpu
